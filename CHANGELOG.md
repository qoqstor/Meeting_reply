...
### jt-live-whisper v2.16.7 (2026-06-25)

**新增 — Whisper medium 模型選項**
- `WHISPER_MODELS` 清單加入 `medium`（faster-whisper CTranslate2 格式，~1.4 GB）
- medium 屬於 `_PROMPT_CAPABLE_MODELS`，`initial_prompt` 自動啟用（小模型因會將 prompt 當辨識結果輸出，刻意停用）
- 離線 bundle 同時內附 small + medium，使用者可在啟動時自行選擇
- 適用場景：CPU-only 機器、日文整句辨識錯誤率偏高時的平衡方案（速度 ~8–12s，優於 large-v3-turbo 的 ~18s）
- `jt_live_whisper.spec` 新增 medium 模型打包路徑

---

### SEI Reply Assistant v2.1.0 (2026-06-25)

**修正 — 非雙向模式（ja2zh 等）監控來源指向舊檔**
- 根因 1：`_find_latest_log()` 只搜 `日中雙向_逐字稿_*.txt`，但 `ja2zh` 模式寫出 `日翻中_逐字稿_*.txt`，導致找不到當日檔案、落回昨日舊檔
- 修法：glob 改為 `*逐字稿_*.txt`，涵蓋所有翻譯模式

**修正 — 非雙向模式翻譯內容不顯示於即時翻譯區**
- 根因 2：`LINE_RE` 要求 `[▶◀]` 方向符號，雙向模式才有此格式；`ja2zh` 等單向模式格式為 `[HH:MM:SS] [日] text`，regex 不匹配 → 解析 0 筆
- 修法：`[▶◀]` 改為 optional group `(?:[▶◀]\s*)?`

**改善 — 可調整區塊大小的版面**
- 即時翻譯區與建議回覆區改用 `ttk.PanedWindow(orient="vertical")`，拖曳 sash 可自由調整比例（預設 3:2）
- 建議回覆區移除固定 `height=4`，隨 pane 大小自動伸縮

**調整 — 回覆建議取用範圍與自動觸發間隔**
- `MAX_SENTENCES` 7 → 30（涵蓋更完整的對話脈絡）
- `AUTO_TRIGGER_DEBOUNCE` 3.0s → 8.0s（給本機 LLM 足夠時間產生回覆再觸發下一次）
